...
log "Installing realsense2 packages"
sudo apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-key C8B3A55A6F3EFCDE
sudo add-apt-repository "deb https://librealsense.intel.com/Debian/apt-repo $(lsb_release -cs) main"
sudo apt-get update
sudo apt-get install -y librealsense2-dkms --allow-unauthenticated -y
sudo apt-get install -y librealsense2-dev --allow-unauthenticated -y
sudo apt-get install -y librealsense2-utils --allow-unauthenticated -y
