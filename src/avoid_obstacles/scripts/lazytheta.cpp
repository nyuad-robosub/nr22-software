...
struct LazyTheta {
	...
	vector<pair<Vector3i, float>> GetVisNeighborsXYZ(const Node& s) {
		vector<bool> obs;
		for (int x = s.xyz(0) - 1; x <= s.xyz(0); x++) 
			for (int y = s.xyz(1) - 1; y <= s.xyz(1); y++) 
				for (int z = s.xyz(2) - 1; z <= s.xyz(2); z++)
		...
		if (obs[0] & obs[1] & obs[2] & obs[3]) nbs.push_back(make_pair(s.xyz + Vector3i(-1, 0, 0), NGBR_COST[0]));
		if (obs[4] & obs[5] & obs[6] & obs[7]) nbs.push_back(make_pair(s.xyz + Vector3i(1, 0, 0), NGBR_COST[0]));
		if (obs[0] & obs[1] & obs[4] & obs[5]) nbs.push_back(make_pair(s.xyz + Vector3i(0, -1, 0), NGBR_COST[0]));
		if (obs[2] & obs[3] & obs[6] & obs[7]) nbs.push_back(make_pair(s.xyz + Vector3i(0, 1, 0), NGBR_COST[0]));
		if (obs[0] & obs[2] & obs[4] & obs[6]) nbs.push_back(make_pair(s.xyz + Vector3i(0, 0, -1), NGBR_COST[0]));
		if (obs[1] & obs[3] & obs[5] & obs[7]) nbs.push_back(make_pair(s.xyz + Vector3i(0, 0, 1), NGBR_COST[0]));

		// Column for diagonal points
		if (obs[0] & obs[2]) nbs.push_back(make_pair(s.xyz + Vector3i(-1, 0, -1), NGBR_COST[1]));
		if (obs[4] & obs[6]) nbs.push_back(make_pair(s.xyz + Vector3i(1, 0, -1), NGBR_COST[1]));
		if (obs[0] & obs[4]) nbs.push_back(make_pair(s.xyz + Vector3i(0, -1, -1), NGBR_COST[1]));
		if (obs[2] & obs[6]) nbs.push_back(make_pair(s.xyz + Vector3i(0, 1, -1), NGBR_COST[1]));
		if (obs[0] & obs[1]) nbs.push_back(make_pair(s.xyz + Vector3i(-1, -1, 0), NGBR_COST[1]));
		if (obs[4] & obs[5]) nbs.push_back(make_pair(s.xyz + Vector3i(1, -1, 0), NGBR_COST[1]));
		if (obs[2] & obs[3]) nbs.push_back(make_pair(s.xyz + Vector3i(-1, 1, 0), NGBR_COST[1]));
		if (obs[6] & obs[7]) nbs.push_back(make_pair(s.xyz + Vector3i(1, 1, 0), NGBR_COST[1]));
		if (obs[1] & obs[3]) nbs.push_back(make_pair(s.xyz + Vector3i(-1, 0, 1), NGBR_COST[1]));
		if (obs[5] & obs[7]) nbs.push_back(make_pair(s.xyz + Vector3i(1, 0, 1), NGBR_COST[1]));
		if (obs[1] & obs[5]) nbs.push_back(make_pair(s.xyz + Vector3i(0, -1, 1), NGBR_COST[1]));
		if (obs[3] & obs[7]) nbs.push_back(make_pair(s.xyz + Vector3i(0, 1, 1), NGBR_COST[1]));

		// Block for corner points
		if (obs[0]) nbs.push_back(make_pair(s.xyz + Vector3i(-1, -1, -1), NGBR_COST[2]));
		if (obs[1]) nbs.push_back(make_pair(s.xyz + Vector3i(-1, -1, 1), NGBR_COST[2]));
		if (obs[2]) nbs.push_back(make_pair(s.xyz + Vector3i(-1, 1, -1), NGBR_COST[2]));
		if (obs[3]) nbs.push_back(make_pair(s.xyz + Vector3i(-1, 1, 1), NGBR_COST[2]));
		if (obs[4]) nbs.push_back(make_pair(s.xyz + Vector3i(1, -1, -1), NGBR_COST[2]));
		if (obs[5]) nbs.push_back(make_pair(s.xyz + Vector3i(1, -1, 1), NGBR_COST[2]));
		if (obs[6]) nbs.push_back(make_pair(s.xyz + Vector3i(1, 1, -1), NGBR_COST[2]));
		if (obs[7]) nbs.push_back(make_pair(s.xyz + Vector3i(1, 1, 1), NGBR_COST[2]));
		// if (obs[]) nbs.push_back(make_pair(s.xyz + Vector3i(, , ), NGBR_COST[2]));
		return nbs;
	}

	// Implementing fast ray traversal algorithm
    // http://www.cse.yorku.ca/~amana/research/grid.pdf
	bool LineOfSight(const Node& start, const Node& end) {
		cout << "los " << start.xyz(0) << " " << start.xyz(1) << " " << start.xyz(2) << " " << endl;
		auto nbs = GetVisNeighborsXYZ(start);
		for (pair<Vector3i, float> i: nbs)
			if (i.first == end.xyz)
				return true;
		
		// Ray traversing from end to start
		Vector3i vi = start.xyz - end.xyz;
		...
		if (vi.all()) {
			// All is well
			// Define start & end
			Vector3i startVoxel(end.xyz + (Vector3i(-1, -1, -1) + step) / 2);
			Vector3i endVoxel(start.xyz + (Vector3i(-1, -1, -1) - step) / 2);
			...
			if (tripleMove) {
				cout << "triplemove 1 " << XYZ(0) << " " << XYZ(1) << " " << XYZ(2) << " " << endl;
				while (XYZ != endVoxel) {
					XYZ += step;
					if (blockedXYZ.find(XYZ) != blockedXYZ.end())
						return false;
				...
				cout << "doublemove 1 " << XYZ(0) << " " << XYZ(1) << " " << XYZ(2) << " " << endl;
				while (XYZ != endVoxel) {
					float tMaxMin = tMax.minCoeff();
					// If the two doubleMove axes are min -> prioritize
					if (tMax[dMAx] == tMaxMin || (tMax[dMAx] == tMax[dMAxes(0)] && prioritizeDoubleMove)) {
						for (int i = 0; i < 2; i++) {
							XYZ(i) += step(i);
							tMax(i) += tDelta(i);
						}
				...
				cout << "normalmove 1 " << XYZ(0) << " " << XYZ(1) << " " << XYZ(2) << " " << endl;
			...
			Vector3i startFace(end.xyz + (Vector3i(-1, -1, -1) + step) / 2);
			Vector3i endFace(start.xyz + (Vector3i(-1, -1, -1) - step) / 2);
			startFace(ax) = end.xyz(ax);
			endFace(ax) = start.xyz(ax);
			...
			for (int i = 0; i < 3; i++)
				if (startFace(i) == startFace(i)) tMax(i) = numeric_limits<float>::max();
			...
			if (doubleMove) {
				cout << "doublemove 2 " << XYZ[0](0) << " " << XYZ[0](1) << " " << XYZ[0](2) << " " << endl;
				while (XYZ[0] != endFace) {
					for (int i = 0; i < 2; i++) {
						XYZ[i] += step;
					}
				...
				cout << "normalmove 2 " << XYZ[0](0) << " " << XYZ[0](1) << " " << XYZ[0](2) << " " << endl;
				int tMaxMin = 0;
				float min_tMax = 0;
				while (XYZ[0] != endFace) {
			...
			Vector3i startEdge(end.xyz);
			Vector3i endEdge(start.xyz);
			...
			cout << "normalmove 3 " << XYZ[0](0) << " " << XYZ[0](1) << " " << XYZ[0](2) << " " << endl;
			while (XYZ[0] != endEdge) {
				for (int i = 0; i < 4; i++)
					XYZ[i] += step;
	...
	void SetVertex(Node& s) {
		...
		if (!LineOfSight(*s.parent, s)) {
			auto nbs = GetVisNeighborsXYZ(s);
		...
		while (!open.empty()) {
			...
			for (pair<Vector3i, float> i: nbs) {
				...
				if (closed.find(i.first) == closed.end()) {
					...
					Node* s_apos;
					if (search == open.end()) {
						nodes.push_back(Node(i.first, &s_end));
						s_apos = &(*(nodes.end() - 1));
					} else {
...
int main() {
	...
	for (int i = 0; i < obst.size(); i++) {
		for (int x = obst[i](0) - size; x <= obst[i](0) + size; x++)
			for (int y = obst[i](1) - size; y <= obst[i](1) + size; y++)
				for (int z = obst[i](2) - size; z <= obst[i](2) + size; z++) {
